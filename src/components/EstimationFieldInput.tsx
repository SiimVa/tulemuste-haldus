...
export function EstimationFieldInput({ field, value, onChange, disabled }: { field: PointField; value: string; onChange: (value: string) => void; disabled?: boolean }) {
  ...
  return <fieldset className="min-w-0 space-y-2 rounded border p-2" disabled={disabled || config.targets.some(t => t.correct === null)}>
    <legend className="text-xs text-gray-500">Võistkonna pakkumised ({config.unit})</legend>
    {config.targets.some(t => t.correct === null) && <p className="text-sm text-amber-700">Õiged kaugused tuleb enne pakkumiste sisestamist määrata.</p>}
    {result.rows.map(row => <div className="space-y-1" key={row.id}>
      <label className="flex items-center gap-2 text-sm"><span className="min-w-0 flex-1">{row.label}</span><input aria-label={`${field.label}: ${row.label}`} type="number" min="0" step="any" value={guesses[row.id] ?? ""} onWheel={e => e.currentTarget.blur()} onChange={e => {
      ...
      {row.points !== null && <p className="text-xs text-gray-600">Viga {formatEstimate(row.error!)} {config.unit} · {formatEstimate(row.errorPercent!)}% · {row.points} p</p>}
    </div>)}
