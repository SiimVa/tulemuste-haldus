...
import { estimateNumber, readEstimation } from "@/lib/estimation"
...
export function JudgeEstimationSettings({ elementId, field, accessToken }: { elementId: string; field: PointField & { id: string }; accessToken?: string }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage("Õiged kaugused salvestatud ja tulemused uuesti arvutatud.")
      router.refresh()
  ...
  return <details className="m-3 rounded border bg-blue-50/40 p-3">
    <summary className="cursor-pointer text-sm font-medium">Õiged kaugused: {field.label}</summary>
    <form onSubmit={save} className="mt-3 space-y-2">
      ...
      {config.targets.map(target => <label key={target.id} className="flex items-center gap-2 text-sm"><span className="min-w-0 flex-1">{target.label} ({config.unit})</span><input aria-label={`Õige kaugus: ${target.label}`} type="number" step="any" min="0.000000001" required value={values[target.id] ?? ""} onChange={e => setValues({ ...values, [target.id]: e.target.value })} className="w-28 min-w-0 rounded border px-2 py-1" /></label>)}
      <button disabled={saving} className="rounded bg-blue-600 px-3 py-2 text-sm text-white">{saving ? "Salvestan…" : "Salvesta õiged kaugused"}</button>
