...
export type AnalysisPageAccess =
  | { type: "PUBLIC"; competitionId: string }
  | { type: "LINK_ONLY"; analysisLinkToken: string }

// Leiab võistluse ainult siis, kui valitud ligipääsuviis seda lubab.
// PRIVATE-režiimis ei leia kumbki haru midagi ja leht annab 404.
function analysisWhere(access: AnalysisPageAccess) {
  return access.type === "PUBLIC"
    ? { id: access.competitionId, analysisAccessMode: "PUBLIC" }
    : {
        analysisAccessMode: "LINK_ONLY",
        analysisTokenHash: hashAnalysisLinkToken(access.analysisLinkToken),
      }
}

// Võistlusega seotud kasutajad (korraldajad, kohtunikud, vaatlejad) näevad analüüsi
// ka siis, kui avalik ligipääs on LINK_ONLY või PRIVATE.
async function canViewAsMember(competitionId: string) {
...
export async function analysisPageMetadata(access: AnalysisPageAccess) {
  ...
  const comp = await prisma.competition.findFirst({
    where: analysisWhere(access),
    select: { name: true },
  })
  return { title: comp ? `${comp.name} – Analüüs` : "Analüüs" }
}

export async function PublicAnalysisPage({ access }: { access: AnalysisPageAccess }) {
  if (access.type === "LINK_ONLY" && !isAnalysisLinkToken(access.analysisLinkToken)) {
    notFound()
  }

  let competition = await prisma.competition.findFirst({ where: analysisWhere(access) })
  if (!competition && access.type === "PUBLIC" && (await canViewAsMember(access.competitionId))) {
    competition = await prisma.competition.findUnique({ where: { id: access.competitionId } })
  }
