...
async function handleDELETE(_req: Request, { params }: { params: Promise<{ entryId: string }> }) {
  const session = await auth()
  if (!session?.user?.id) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })
  const { entryId } = await params
  if (!await canAccessMiscEntry(entryId, { id: session.user.id, role: session.user.role })) {
...
export const PATCH = withSecurityRoute("/api/misc-entries/[entryId]", handlePATCH)
export const DELETE = withSecurityRoute("/api/misc-entries/[entryId]", handleDELETE)
